...
# === Проверяем и создаём виртуальное окружение ===
if [ ! -d "$VENV_DIR" ]; then
    if ! command -v "$PYTHON_VERSION" &> /dev/null; then
        echo -e "${RED}❌ Python $PYTHON_VERSION не установлен. Используйте --install для его установки.${NC}"
        exit 1
    fi
    echo -e "${BLUE}🛠️ Создаю виртуальное окружение в папке '$VENV_DIR'...${NC}"
    "$PYTHON_VERSION" -m venv "$VENV_DIR"
else
    echo -e "${BLUE}🔄 Найдено существующее виртуальное окружение. Использую его.${NC}"
fi
